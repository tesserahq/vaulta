...
def asset_service(db):
    """Create a AssetService instance for testing."""
    return AssetService(db)


def test_create_asset(asset_service, setup_user):
    ...
    asset = asset_service.create_asset(asset_data, setup_user.id)
...
def test_get_asset(asset_service, test_asset):
    """Test retrieving an asset by ID."""
    retrieved_asset = asset_service.get_asset(test_asset.id)
...
def test_get_user_assets(
    ...
    assets = asset_service.get_user_assets(test_user.id)
...
def test_update_asset(asset_service, test_asset):
    ...
    updated_asset = asset_service.update_asset(test_asset.id, update_data)
...
def test_delete_asset(asset_service, test_asset):
    """Test deleting an asset."""
    result = asset_service.delete_asset(test_asset.id)

    assert result is True
    assert asset_service.get_asset(test_asset.id) is None
...
def test_search_assets(
    ...
    results = asset_service.search(query)
